...
public class ContainerWithMostWater {
    public static void main(String[] args) {
        ...
        System.out.println(solution.maxArea(x));
    }

    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public int maxArea(int[] height) {
            return recursion(height, 0, 0, height.length - 1);
        }

        private int recursion(int[] height, int ans, int left, int right) {
            while (right > left) {
                int temp = (right - left) * Math.min(height[right], height[left]);
                ans = Math.max(temp, ans);
                if (height[right] >= height[left]) left++;
                else if (height[right] < height[left]) right--;
//                else return Math.max(recursion(height, ans, left + 1, right), recursion(height, ans, left, right - 1));
            }
            return ans;
        }
